...
app.post("/assignedComplaints", async (req, res) => {
  try {
    const { complaintId, agentId, status, agentName } = req.body;

    // Create the assigned complaint with "In Progress" status
    const assignedComplaint = new AssignedComplaint({
      complaintId,
      agentId,
      status: "In Progress", // Set default status for the assigned complaint
      agentName,
    });
    await assignedComplaint.save();

    // Update the status in ComplaintSchema to "In Progress"
    await ComplaintSchema.findByIdAndUpdate(complaintId, { status: "In Progress" });

    // Send success response
    res.status(201).json({ message: "Complaint assigned successfully", assignedComplaint });
  } catch (error) {
    console.error(error);
